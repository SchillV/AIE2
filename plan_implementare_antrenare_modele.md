...
```
AIE2/
├── main.py                                    # scraper — fetches BNR rates
...
└── resources/                                 # gitignored — all generated artifacts
    ├── data/
    │   └── idr_exchange_rates.csv             # generated by main.py
    └── models/                                # generated by pipeline.py / Streamlit app
        ...
        └── diagnostics.png                    # combined diagnostic figure (CLI only)
```
...
- Read `resources/data/idr_exchange_rates.csv` produced by `main.py`
...
Triggered via CLI:
```bash
python pipeline.py [--csv resources/data/idr_exchange_rates.csv]
```
...
6. Call `generate_all_plots()` → regenerate `diagnostics.png` (CLI only)
...
- Mtime-based cache keys: `@st.cache_data` is keyed on file modification time, so the cache auto-invalidates whenever `pipeline.py` writes new files
...
### 7.3 Combined Diagnostic Panel — 2 × 3 (`diagnostics.png`, CLI only)
| Position | Plot |
|----------|------|
